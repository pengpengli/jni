...
const
  ...
  JNI_VERSION_9   = JInt($00090000);
  {$EXTERNALSYM JNI_VERSION_9}
  ...
  JNI_ENOJAVA = -404;          // the dll resource not found
  {$EXTERNALSYM JNI_ENOJAVA}
...
{$ENDIF}
  ...
  JobjectRefType = ^_JobjectType;
  _JobjectType = (JNIInvalidRefType = 0, JNILocalRefType = 1, JNIGlobalRefType = 2, JNIWeakGlobalRefType = 3);
  ...
  JNINativeInterface_ = packed record
    ...
    // JNI 1.6 Features
    GetObjectRefType: function(Env: PJNIEnv;  obj: JObject): JobjectRefType; {$IFDEF MSWINDOWS} stdcall; {$ENDIF} {$IFDEF LINUX} cdecl; {$ENDIF}

    // JNI 9 Module Features
    GetModule: function(Env: PJNIEnv; AClass: JClass): JModule; {$IFDEF MSWINDOWS} stdcall; {$ENDIF} {$IFDEF LINUX} cdecl; {$ENDIF}
  end;
  ...
  JDK1_1InitArgs = packed record
    ...
    vfprintf: function(FP: Pointer; const Format: PAnsiChar; Args: va_list): JInt; {$IFDEF MSWINDOWS} stdcall; {$ENDIF} {$IFDEF LINUX} cdecl; {$ENDIF}
...
type
  ...
  public
    ...
    // JNI 1.6 Features
    function GetObjectRefType(AObject: JObject): JobjectRefType;

    // JNI 9 Module Features
    function GetModule(AClass: JClass): JModule;
  end;
...
begin
  Result := Env^.NewStringUTF(Env, AString);
end;
...
begin
  ...
  Chars := PUTF8Char(Env^.GetStringUTFChars(Env, JStr, IsCopy));
  if Chars = nil then begin
    Result := '';
  end else begin
    Result := UTF8String(Chars);
    Env^.ReleaseStringUTFChars(Env, JStr, PUTF8Char(Chars));
  end;
end;
...
function TJNIEnv.GetModule(AClass: JClass): JObject;
begin
  Result := Env^.GetModule(Env, AClass);
end;
...
function TJNIEnv.CallNonvirtualFloatMethod(Obj: JObject; AClass: JClass; MethodID: JMethodID;
...
function TJNIEnv.GetObjectRefType(AObject: JObject): JobjectRefType;
begin
  Result := Env^.GetObjectRefType(Env, AObject);
end;
...
begin
  ...
  begin
    ...
    ErrMessage := SysErrorMessage(ErrCode);
    raise Exception.CreateFmt('LoadLibrary failed %s', [ErrMessage]);
  end;
